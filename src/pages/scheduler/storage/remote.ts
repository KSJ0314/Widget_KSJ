...
import { DEFAULT_SETTINGS, type ScheduleEvent, type SchedulerSettings } from '@scheduler/types';
...
export const createRemoteStorage = (key: string): SchedulerStorage => {
  ...
  return {
    subscribe(onData, onError) {
      ...
      const snapshot: SchedulerSnapshot = { events: [], settings: DEFAULT_SETTINGS };
      let gotEvents = false;
      let gotSettings = false;
      const emit = () => {
        if (gotEvents && gotSettings) onData({ ...snapshot });
      };
      ...
      const unsubSettings = onSnapshot(
        ...
        snap => {
          ...
          snapshot.settings = { ...DEFAULT_SETTINGS, ...(snap.data()?.settings ?? {}) };
          gotSettings = true;
          emit();
        },
        onError,
